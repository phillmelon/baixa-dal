...
#Informar a pagina inicial e final máximos
PG1=1
PG2=9999
...
def main():
  ...
  parser = argparse.ArgumentParser(prog="Baixa DAL",usage='Informe o ano, mês e dia desejado no formato "AAAA MM DD", ou deixe em branco para baixar o DOE de hoje.',description='Baixa o Diário da Assembleia Legislativa de SP.')
  parser.add_argument('ano', type=int, choices=range(int(ANO)-1, 2051), nargs='?', help='Informe o ano com 4 digitos. Este programa consegue recuperar edições de um espaço da Imprensa Oficial onde ficam guardados as edições dos últimos 5 ou 6 meses. Óbviamente este programa não funciona para obter edições de datas futuras :-p')
  parser.add_argument('mes', type=int, choices=range(1, 13), nargs='?', help='O mês desejado com 1 ou 2 digitos.')
  parser.add_argument('dia', type=int, choices=range(1, 32), nargs='?', help='O dia do mês, com 1 ou 2 digitos.')
  ...
  print("Baixando o Diário do Legislativo do Estado de SP")
  print("Data da edição: " + ano + str(mes).zfill(2) + str(dia).zfill(2) + " (AAAAMMDD)")
